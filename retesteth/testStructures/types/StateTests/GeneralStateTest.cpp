...
StateTestInFilled::StateTestInFilled(spDataObjectMove _data)
{
    spDataObject& data = _data.getPointer();
    m_info = GCP_SPointer<Info>(new Info(data->atKey("_info")));
    m_env = GCP_SPointer<StateTestEnv>(new StateTestEnv(data->atKey("env")));

    // -- Some tests has storage keys/values with leading zeros. Convert it to hex value
    for (auto& spAcc : data.getContent().atKeyUnsafe("pre").getSubObjectsUnsafe())
    {
    ...
    m_pre = spState(new State(data->atKey("pre")));

    m_transaction = GCP_SPointer<StateTestTransaction>(new StateTestTransaction(data->atKey("transaction")));
    for (auto const& elFork : data->atKey("post").getSubObjects())
    {
    ...
    requireJsonFields(data, "StateTestInFilled " + data->getKey(),
