...
string const nethermind_start = R"(#!/bin/sh

wevm="/nethermind/tools/Evm/Evm/bin/Debug/net8.0/Evm"

if [ ! -x "$wevm" ]; then
   >&2 echo "Can't find executable evm at the specified path!"
   exit 1
fi
echo "evm executable found at $wevm"
...
gennethermindt8ncfg::gennethermindt8ncfg()
{
...
string const nethermind_config = R"({
    ...
    string const customcompiler = R"(#!/bin/sh
...
# just like described in this file."
    ...
    }
    {
        ...
        (*obj)["content"] = nethermind_start;
        map_configs.addArrayObject(obj);
    }
    {
        spDataObject obj;
        (*obj)["exec"] = true;
        (*obj)["path"] = "nethermind/yul.sh";
    ...
    }
    {
        spDataObject obj;
        (*obj)["exec"] = true;
        (*obj)["path"] = "nethermind/mycompiler.sh";
        (*obj)["content"] = customcompiler;
        map_configs.addArrayObject(obj);
    }
}
}  // namespace retesteth::options
