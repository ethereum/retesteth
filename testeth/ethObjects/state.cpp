...
{
    bool wasError = false;
    auto markError = [&wasError]() -> string { wasError = true; return ""; };

    for (auto const& a: _stateExpect.getAccounts())
    {
        if (a.shouldNotExist())
            BOOST_CHECK_MESSAGE(!_statePost.hasAccount(a.address()),
                 markError() + TestOutputHelper::get().testName() +  " Compare States: " << a.address()
        ...
        account inState = _statePost.getAccount(a.address());

        if (a.hasBalance())
            BOOST_CHECK_MESSAGE(a.getData().at("balance").asString() == inState.getData().at("balance").asString(),
                 markError() + TestOutputHelper::get().testName() + " Check State: " << a.address()
        ...
        if (a.hasStorage())
            a.compareStorage(_statePost.getData().at("storage"));
